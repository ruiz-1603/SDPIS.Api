...
export function FormularioDenuncia() {
    ...
    // Heurística 3 (Control y libertad del usuario): tras confirmar, la
    // persona debe poder volver al inicio sin quedar "atrapada" en la
    // pantalla de éxito.
    function manejarVolverAlInicio() {
        reiniciarFormulario();
        setPasosConIntento(new Set());
        setNumeroDenuncia(null);
    }
    ...
    if (numeroDenuncia) {
        return (
            <ConfirmacionDenuncia
                ...
                denunciante={estado.denunciante}
                onVolverAlInicio={manejarVolverAlInicio}
            />
        );
    }
            ...
            <div className="progress-wrap">
                <div className="progress-track">
                    {PASOS.map((paso) => (
                        <div
                            key={paso.numero}
                            className={`step${estado.paso === paso.numero ? ' active' : ''}${estado.paso > paso.numero ? ' done' : ''
                                }`}
                        >
